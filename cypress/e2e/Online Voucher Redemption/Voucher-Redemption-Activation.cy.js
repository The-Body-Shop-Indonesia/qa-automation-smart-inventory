...
describe('Voucher Partnership Purchase Attempt Testing', () => {
  ...
  it('My Point should correctly deducted', () => {
    ...
    cy.api({
      ...
      .should((response) => {
        ...
        expect(
          pointAfterDeduction,
          'My Point is correctly deducted'
        ).to.be.equal(pointAfterPurchase)
  ...
  it('Shows Purchased Voucher in My Voucher List', () => {
    ...
    cy.api({
      ...
      partnership_voucher.forEach(function (item) {
        ...
        if (
          item.tbsCodeVoucherProgram ===
          Cypress.env('VOUCHER_TO_BUY').tbsCodeVoucherProgram
        ) {
          expect(item.redeemDate).to.be.null
  ...
  it('failed to buy voucher when voucher detail is exceeded', () => {
    //get expired voucher program
    const partnerCode = 'QA-TEST-AUTO-PARTNER'

    cy.api({
      method: 'GET',
      url:
        Cypress.env('VMS_URL') +
        `?page=1&limit=50&sort=-updatedAt&keywords=${partnerCode}`,
      headers: Cypress.env('REQUEST_HEADERS_ADMIN')
    }).then((vcrresponse) => {
      const unaffordableVouchers = vcrresponse.body.data.docs.filter((vcr) => {
        const redeemable = vcr.isRedeemable
        const partnership = vcr.isPartnerShip
        return redeemable === true && partnership === true
      })
      ...
      Cypress.env('VOUCHER_TO_BUY', unaffordableVouchers[0])
      cy.log(
        `attempt to purchase voucherID ${voucherToBuy.tbsCodeVoucherProgram}`
      )

      cy.api({
        method: 'GET',
        url:
          Cypress.env('VMS_URL_DETAIL') +
          `?page=1&limit=10&keywords=${Cypress.env('VOUCHER_TO_BUY').tbsCodeVoucherProgram}`,
        headers: Cypress.env('REQUEST_HEADERS_ADMIN')
      }).then((detailresponse) => {
        ...
        if (data.totalDocs === 0) {
          ...
          cy.api({
            ...
            failOnStatusCode: false,
            body: {
              voucherId: Cypress.env('VOUCHER_TO_BUY')._id
            }
          }).should((_response) => {
            expect(_response.status, 'Expect Http status 400').to.equal(400)
          })
        } else {
          cy.log('Voucher detail detected')
        }
      })
    })
  })
...
describe('Voucher TBS Purchase Attempt Testing', () => {
  ...
  it('Success to buy voucher with sufficient point', () => {
    ...
    cy.api({
      ...
      const voucherToBuy = affordableVouchers[0]
      Cypress.env('VOUCHER_TO_BUY_NONPARTNER', affordableVouchers[0])
  ...
  it('My Point should correctly deducted', () => {
    ...
    cy.api({
      ...
      const pointAfterDeduction = response.body.data.currentPoint
      const purchasedVoucher = Cypress.env('VOUCHER_TO_BUY_NONPARTNER')
      console.log(purchasedVoucher.redeemPoint)
      const voucherPrice = purchasedVoucher.redeemPoint
      console.log('Voucher price: ', purchasedVoucher.redeemPoint)
      const pointAfterPurchase = myPoint - voucherPrice
  ...
  it('Shows Purchased Voucher in My Voucher List', () => {
    ...
    cy.api({
      ...
      expect(response.status).to.equal(200)
      const active_voucher = response.body.data.activeVouchers
      expect(
        active_voucher.length,
        'Purchased Voucher exist in my voucher'
      ).to.be.greaterThan(0)
      active_voucher.forEach(function (item) {
        expect(item).to.haveOwnProperty('voucherCode')
        expect(item).to.haveOwnProperty('tbsCodeVoucherProgram')
        expect(item).to.haveOwnProperty('type')
        expect(item).to.haveOwnProperty('title')
        ...
        if (
          item.tbsCodeVoucherProgram ===
          Cypress.env('VOUCHER_TO_BUY_NONPARTNER').tbsCodeVoucherProgram
        ) {
          expect(item.redeemDate).to.be.null
          expect(item.available).to.be.true
          expect(item.status).to.equal('available')
  ...
  it('Quota voucher deducted', () => {
    // quota from voucher redemption list
    //const URL_PRODUCT = Cypress.config('baseUrlProduct')
    const voucher_program = Cypress.env(
      'VOUCHER_TO_BUY_NONPARTNER'
    ).tbsCodeVoucherProgram
    ...
    cy.api({
      ...
      const data = response.body.data[0]
      const quota_awal = Cypress.env('VOUCHER_TO_BUY_NONPARTNER').quota
      const quota_calculate = quota_awal - 1
      expect(data.quota).to.be.equal(quota_calculate)
    })
  })
