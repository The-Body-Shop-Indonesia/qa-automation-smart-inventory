...
const url = baseUrl + '/stock-movement'
...
describe('API Test - Valid & invalid', () => {
  ...
  it('Should contain correct API valid token', () => {
    cy.api({
      method: 'GET',
      url,
...
describe('Filter Test Group', () => {
  ...
  it('Should return the correct orderNumber', () => {
    ...
    cy.api({
      ...
      const data = response.body.data.docs
      expect(Cypress._.every(data, ['orderNumber', orderNumber])).to.deep.equal(
        true
      )
    })
  })
  ...
  it('Should return the correct ubd when ubd is null', () => {
    const ubd = 'null'
    const urlFilter = url + `?ubd=${ubd}&page=1&limit=100`
  ...
  it('Should return the correct SKU & Order Number', () => {
    ...
    const urlFilter =
      url + `?sku=${sku}&orderNumber=${orderNumber}&page=1&limit=100` // Menyertakan filter 'to' dalam URL
    cy.api({
      method: 'GET',
      url: urlFilter,
      headers: Cypress.env('REQUEST_HEADERS'), // Gunakan header yang diambil dari login
      ...
      if (data.length > 0) {
        expect(Cypress._.every(data, ['sku', sku])).to.deep.equal(true)
        expect(
          Cypress._.every(data, ['orderNumber', orderNumber])
        ).to.deep.equal(true)
      } else {
        throw new Error('No documents found in the response')
      }
  ...
  it('Should return the correct SKU + UBD + Order Number', () => {
    ...
    const urlFilter =
      url + `?sku=${sku}&ubd=${UBD}&orderNumber=${orderNumber}&page=1&limit=100` // Menyertakan filter 'to' dalam URL
    cy.api({
      method: 'GET',
      url: urlFilter,
      headers: Cypress.env('REQUEST_HEADERS'), // Gunakan header yang diambil dari login
  ...
  it('The item returned should match the sort Ascending', () => {
    cy.api({
      ...
      const sortedStockMovement = stockMovement.sort(
        (a, b) => new Date(a.updatedAt) - new Date(b.updatedAt)
      )

      // Step 5: Log the sorted results for visibility
      cy.log(
        'Sorted stock movements by updatedAt (Ascending): ',
        JSON.stringify(sortedStockMovement, null, 2)
      )
      ...
      if (sortedStockMovement.length > 0) {
        ...
        throw new Error(
          'No stock movement data found for SKU 112010666 from 14036'
        )
      }
      ...
      for (let i = 0; i < sortedStockMovement.length - 1; i++) {
        if (
          new Date(sortedStockMovement[i].updatedAt) >
          new Date(sortedStockMovement[i + 1].updatedAt)
        ) {
          isSortedAscending = false
          break
        }
      }
      expect(isSortedAscending).to.eq(
        true,
        'The stock movements should be sorted in ascending order by updatedAt'
      )
    })
  })
})
