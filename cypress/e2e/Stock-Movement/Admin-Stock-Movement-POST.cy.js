  ...
  it('Should return the correct API and Verify correct Stock Movement - Sales', () => {
    ...
    cy.api({
      ...
      if (!latestStock) {
        ...
        cy.log(
          `SKU 134070359 at from store 14160 - Previous totalStock: ${Before_latestStock_Movement}, Current totalStock: 0`
        )
      } else {
      ...
      cy.api({
        ...
        if (!latestStock) {
          ...
          cy.log(
            `SKU ${sku_sales} at from store ${store_code_sales} - Previous totalStock: ${Before_latestStock_Summary}, Current totalStock: 0`
          )
        } else {
        ...
        cy.api({
          ...
          cy.api({
            ...
            if (!latestStock) {
              ...
              throw new Error(
                `No stock data found for ${sku_sales} at from store ${store_code_sales} with UBD ${ubd1}`
              )
            }
            ...
            cy.api({
              ...
              if (!latestStock) {
                ...
                throw new Error(
                  `No stock data found for ${sku_sales} at ${store_code_sales} with ${ubd1}`
                )
              }
              ...
              cy.log(
                'Total Stock Movement [Kondisi setelah Movement]: ',
                After_latestStock_Movement
              )
              cy.log(
                'Qty Stock Summary [Kondisi setelah Movement]: ',
  ...
  it('Should return the correct API and Verify correct Stock Movement - Adjustment-IN', () => {
    ...
    cy.api({
      ...
      if (!latestStock) {
        ...
        cy.log(
          `SKU ${sku_adjustment} at from store${store_code_adjustment} - Previous totalStock: ${Before_latestStock_Movement}, Current totalStock: 0`
        )
      } else {
        Before_latestStock_Movement = latestStock.totalStock // Simpan nilai ini di variabel luar
      }
      ...
      cy.api({
        ...
        if (!latestStock) {
          Before_latestStock_Summary = 0 // Sekarang Before_latestStock_Summary di-set di sini
          cy.log(
            `SKU ${sku_adjustment} at from store ${store_code_adjustment} - Previous totalStock: ${Before_latestStock_Summary}, Current totalStock: 0`
          )
        } else {
          Before_latestStock_Summary = latestStock.qty // Simpan nilai ini di variabel luar
        }
        ...
        cy.api({
          ...
          cy.api({
            ...
            if (!latestStock) {
              cy.log('Available Stocks:', stockMovement) // Log semua data stok yang tersedia
              throw new Error(
                `No stock movement data found for SKU ${sku_adjustment} at from store ${store_code_adjustment} with UBD ${ubd2}`
              )
            }
            // Validasi data GET dengan data POST
            ...
            cy.api({
              ...
              if (!latestStock) {
                cy.log('Available Stocks:', stockSummary) // Log semua data stok yang tersedia
                throw new Error(
                  `No stock summary data found for SKU ${sku_adjustment} at store ${store_code_adjustment} with UBD ${ubd2}`
                )
              }
...
describe(' Create Stock Movement - UBD Null', () => {
  let requestBodyUBDNULL // Menyimpan body request POST
  let postDataUBDNULL // Menyimpan body request POST
  it('Should return the correct API and Verify correct Stock Movement - sales - UBD NULL', () => {
    const urlGet = `${URL_PRODUCT}/admin/stock-summary` // URL endpoint GET
    const urlGet2 = `${URL_PRODUCT}/admin/stock-movement` // URL endpoint GET
    const urlPost = `${URL_PRODUCT}/admin/stock-movement` // URL endpoint POST
    //Before Stock Movement Condition
    cy.api({
      ...
      const latestStock = stockMovement.filter(
        (item) => item.sku === sku_sales && item.from === 'DC'
      )[0]
      ...
      cy.api({
        ...
        const latestStock = stockSummary.filter(
          (item) =>
            item.sku === sku_sales && item.storeCode === store_code_sales
        )[0]
        ...
        cy.api({
          ...
          // Validasi properti 'data' lebih detail jika diperlukan
          expect(postDataUBDNULL).to.have.property('sku', `${sku_sales}`)
          expect(postDataUBDNULL).to.have.property(
            'name',
            'SHEA BODY BUTTER 200ML'
          )
          ...
          cy.api({
            ...
            // Cari item dengan SKU yang sesuai dan filter berdasarkan UBD, sesuaikan dengan format bulan dan tahun
            const latestStock = stockMovement.filter(
              (item) => item.sku === sku_sales && item.from === store_code_sales
            )[0] //&&
            ...
            if (!latestStock) {
              cy.log('Available Stocks:', stockMovement) // Log semua data stok yang tersedia
              throw new Error(
                `No stock movement data found for SKU ${sku_sales} at from store ${store_code_sales} with UBD NULL`
              )
            }
            // Validasi data GET dengan data POST
            ...
            expect(latestStock.orderNumber, 'OrderNumber').to.eq(
              requestBodyUBDNULL.orderNumber
            )
            ...
            cy.api({
              ...
              if (!latestStock) {
                cy.log('Available Stocks:', stockSummary) // Log semua data stok yang tersedia
                throw new Error(
                  `No stock summary data found for SKU ${sku_sales} at store ${store_code_sales} with UBD Null`
                )
              }
  ...
  it('Should return the correct API and Verify correct Stock Movement - Adjustment-IN - UBD NULL', () => {
    const urlGet = `${URL_PRODUCT}/admin/stock-summary` // URL endpoint GET
    const urlGet2 = `${URL_PRODUCT}/admin/stock-movement` // URL endpoint GET
    const urlPost = `${URL_PRODUCT}/admin/stock-movement` // URL endpoint POST
    //Before Stock Movement Condition
    cy.api({
      ...
      // Cari item dengan SKU yang sesuai dan filter berdasarkan UBD, sesuaikan dengan format bulan dan tahun
      const latestStock = stockMovement.filter(
        (item) => item.sku === sku_adjustment && item.from === 'DC'
      )[0]
        ...
        cy.api({
          ...
          cy.api({
            ...
            if (!latestStock) {
              cy.log('Available Stocks:', stockMovement) // Log semua data stok yang tersedia
              throw new Error(
                `No stock movement data found for SKU ${sku_adjustment} at from store ${store_code_adjustment} with UBD NULL`
              )
            }
            // Validasi data GET dengan data POST
            expect(latestStock.sku, 'SKU').to.eq(requestBodyUBDNULL.sku) // Mengambil dari requestBody
            ...
            cy.api({
              ...
              if (!latestStock) {
                cy.log('Available Stocks:', stockSummary) // Log semua data stok yang tersedia
                throw new Error(
                  `No stock summary data found for SKU ${sku_adjustment} at store ${store_code_adjustment} with UBD Null`
                )
              }
  ...
  it('Should return the correct API and Verify correct Stock Movement - Adjustment-OUT - UBD NULL', () => {
    const urlGet = `${URL_PRODUCT}/admin/stock-summary` // URL endpoint GET
    const urlGet2 = `${URL_PRODUCT}/admin/stock-movement` // URL endpoint GET
    const urlPost = `${URL_PRODUCT}/admin/stock-movement` // URL endpoint POST
    //Before Stock Movement Condition
