...
const URL_PRODUCT_REDEMPTION = URL_PRODUCT + '/employee/cart-redemption/item' //old version
const URL_REDEMPTION_ORDER = URL_PRODUCT + '/order-redemption'

const sku_redemption_1 = '112620556'
const ubd_redemption_1 = 'null'
...
describe('Get last product stock on Stock Summary and Stock Movement', function () {
  ...
  //stock summary
  it(`Get product stock from Stock Summary ${sku_redemption_1} with UBD null`, () => {
  ...
  //stock movement
  it(`Get product stock from Stock Movement ${sku_redemption_1} with UBD null`, () => {
...
describe('Cart Redemption Section', function () {
  it(`Add product Redemption product Redemption ${sku_redemption_1}`, () => {
    const cartId = Cypress.env('CART_ID')
    const qtyProduct = 1
    cy.api({
      method: 'PATCH',
  ...
  it('Checkout Redemption order', () => {
    ...
    cy.api({
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        expect(
          items[i].ubdDetail[0].ubd,
          `UBD ${sku_redemption_1} with UBD ${ubd_redemption_1} should be null`
        ).to.equal(null)
        expect(
          items[i].ubdDetail[0].total,
