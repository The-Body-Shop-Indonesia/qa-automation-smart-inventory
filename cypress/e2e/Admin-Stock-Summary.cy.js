...
describe('Filter Test Group', () => {
  ...
  it('Should return error if request incorrect UBD format', () => {
    ...
    cy.request({
      ...
      expect(body.message[0]).to.equal(
        'ubd must match /^(1[0-9]{3}|2[0-9]{3})-(0[0-9]|1[0-2])-([0-2][0-9]|3[0-1])$/ regular expression'
      )
    })
  })
  ...
  it('Should return the correct result filtered by SKU + UBD + Store Code', () => {
    ...
    const sku = '112620556'
    const urlFilter =
      url + `?page=1&limit=10&sku=${sku}&storeCode=${storeCode}&ubd=${ubd}`
    cy.request({
      method: 'GET',
      url: urlFilter,
      headers: Cypress.env('REQUEST_HEADERS')
