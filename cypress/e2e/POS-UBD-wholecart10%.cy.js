...
describe('Staff create order', function () {
  ...
  it('Should able to add product to cart by scan QR', () => {
    ...
    cy.api({
      ...
      .then((response) => {
        ...
        paymentDetailSubtotal = subtotalDetail.total
        ...
        PN_Cart = response.body.data.items[0].promoNumber
        ...
        paymentAmount = response.body.data.paymentAmount
        ...
        cy.log(
          expected_priceSKU_afterDisc,
          'Expected harga setelah disc per SKU'
        )
        ...
        DetailPromoCart = TotalPromo.total
        cy.log('Total Promo:', DetailPromoCart)

        const expectedGrandTotal = subtotalDetail.total + DetailPromoCart
  ...
  it('Successfully create order', () => {
    ...
    cy.api({
      ...
      expect(body.customer.id, 'Customer ID:').to.equal(customerId)
      ...
      const promoDetail = response.body.data.payments?.breakDown?.find(
        (detail) => detail.label === 'Promo'
      )
      const paymentDetailPromo = promoDetail.total
      ...
      expect(body.items[0].promoNumber, 'Promo Number').to.equal(PN_Cart)
      expect(body.totalAmount, 'Total Amount').to.equal(totalAmount)
      expect(paymentDetailPromo, 'Promo Detail').to.equal(DetailPromoCart)
      expect(body.paymentAmount, 'Payment Amount').to.equal(paymentAmount)
    })
  })
})
