  ...
  it('Should contain correct API response format', () => {
    cy.api({
      ...
      .should((response) => {
        ...
        expect(
          Cypress._.every(docs, (docs) => docs.hasOwnProperty('_id')),
          'All data has _id property'
        ).to.equal(true)
        expect(
          Cypress._.every(docs, (docs) => docs.hasOwnProperty('items_count')),
          'All docs has items_count property'
        ).to.equal(true)
        expect(
          Cypress._.every(docs, (docs) => docs.hasOwnProperty('total_price')),
          'All docs has total_price property'
        ).to.equal(true)
        expect(
          Cypress._.every(docs, (docs) => docs.hasOwnProperty('customer_name')),
          'All docs has customer_name property'
        ).to.equal(true)
        expect(
          Cypress._.every(docs, (docs) => docs.hasOwnProperty('customer_nik')),
          'All docs has customer_nik property'
        ).to.equal(true)
        expect(
          Cypress._.every(docs, (docs) => docs.hasOwnProperty('customer_id')),
          'All docs has customer_id property'
        ).to.equal(true)
        expect(
          Cypress._.every(docs, (docs) => docs.hasOwnProperty('card_number')),
          'All docs has card_number property'
        ).to.equal(true)
        expect(
          Cypress._.every(docs, (docs) => docs.hasOwnProperty('cart_type')),
          'All docs has cart_type property'
        ).to.equal(true)
        expect(
          Cypress._.every(docs, (docs) => docs.hasOwnProperty('isFamily')),
          'All docs has isFamily property'
        ).to.equal(true)
        expect(
          Cypress._.every(docs, (docs) => docs.hasOwnProperty('isScanner')),
          'All docs has isScanner property'
        ).to.equal(true)
      })
  })
})
...
describe('Pagination Test Group', () => {
  it('Should be able to apply pagination', () => {
    ...
    cy.request({
      ...
      const data = response.body.data
      expect(data.docs.length, `Docs length data should be ${limit}`).to.equal(
        limit
      )
  ...
  it('The item returned should match the limit set', () => {
    ...
    cy.request({
      ...
      const data = response.body.data
      expect(data.docs.length, `Docs length data should be ${limit1}`).to.equal(
        limit1
      )
    ...
    cy.api({
      ...
      .should((response) => {
        ...
        expect(
          data.docs.length,
          `Docs length data should be ${limit2}`
        ).to.equal(limit2)
...
describe('Skip Normal and Redemption Cart Test Group', () => {
  ...
  it('Should be able to show only redemption cart', () => {
    ...
    const paginationUrl =
      url +
      `?page=${page}&size=${limit}&skipCart=${skip_cart}&skipRedemption=${skip_redemption}`
    cy.api({
      method: 'GET',
      url: paginationUrl,
      headers: Cypress.env('REQUEST_HEADERS')
