...
describe('Get last product stock on Stock Summary and Stock Movement', function() {
  ...
  it(`Get product stock from Stock Summary ${sku_redemption_1}`, () => {
    ...
    const urlFilter = url + `?sku=${sku_redemption_1}&page=1&limit=100&ubd=${ubd_redemption_1}&storeCode=${store_code}`
    cy.request({
      method: "GET",
      url: urlFilter,
  ...
  it(`Get product stock from Stock Summary ${sku_redemption_2}`, () => {
    ...
    const url = URL_PRODUCT + '/admin/stock-summary'
    const urlFilter = url + `?sku=${sku_redemption_2}&page=1&limit=100&ubd=${ubd_redemption_2}&storeCode=${store_code}`
    cy.request({
      method: "GET",
      url: urlFilter,
      headers: Cypress.env("REQUEST_HEADERS_ADMIN")
  ...
  it(`Get product stock from Stock Movement ${sku_redemption_1}`, () => {
    ...
    const urlFilter = url + `?sku=${sku_redemption_1}&page=1&limit=10&sort=${sort_desc}&ubd=${ubd_redemption_1}&from=${store_code}`
    cy.request({
      method: "GET",
      url: urlFilter,
      headers: Cypress.env("REQUEST_HEADERS_ADMIN")
  ...
  it(`Get product stock from Stock Movement ${sku_redemption_2}`, () => {
    const url = URL_PRODUCT + '/admin/stock-movement'
    const urlFilter = url + `?sku=${sku_redemption_2}&page=1&limit=10&sort=${sort_desc}&ubd=${ubd_redemption_2}&from=${store_code}`
    cy.request({
      method: "GET",
      url: urlFilter,
      headers: Cypress.env("REQUEST_HEADERS_ADMIN")
...
describe('Staff Add Cart Redemption for Member Customer', function() {
  ...
  it("Get user point before redemption", () => {
    const url = URL_USER + "/employee/detail-member/point" + `?cardNumber=${card_number}`
    cy.api({
      method: "POST",
      url,
      headers: Cypress.env("REQUEST_HEADERS")
  ...
  it("Create Redemption Cart", () => {
    ...
    .should(response => {
      ...
      expect(data.customer.firstName, 'First name should be ' + `${first_name}`).to.equal(first_name)
      expect(data.customer.lastName, 'Last name should be ' + `${last_name}`).to.equal(last_name)
      expect(data.customer.isGuest, 'isGuest should be false').to.equal(false)
      expect(data.customer.cardNumber, 'Card number should be ' + `${card_number}`).to.equal(card_number)
    ...
    .should(response => {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if(items[i].sku === sku_redemption_1){
          expect(items[i].sku, `SKU should be ${sku_redemption_1}`).to.equal(sku_redemption_1)
          expect(items[i].qty, 'QTY Produk should be 1').to.equal(1)

          const ubdDetails_length = items[i].ubdDetail.length
          expect(items[i].ubdDetail, `UBD Detail for ${sku_redemption_1} should be 1`).to.have.length(1)
      ...
      const calculate_pts = pointsArray.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
      console.log('calculate_pts: ' + calculate_pts);
      expect(data.total_redemption_point, `Total Redemption Catalog should be ${calculate_pts}`).to.equal(calculate_pts)
    })
  })

    it("Add product redemption UBD null", () => {
    const cartId = Cypress.env("CART_ID")
    const qtyProduct = 1
    ...
    .should(response => {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if(items[i].sku === sku_redemption_2){
          expect(items[i].sku, `SKU should be ${sku_redemption_2}`).to.equal(sku_redemption_2)
          expect(items[i].qty, 'QTY Produk should be 1').to.equal(1)

          const ubdDetails_length = items[i].ubdDetail.length
          expect(items[i].ubdDetail, `UBD Detail for ${sku_redemption_2} should be 1`).to.have.length(1)
          for(let j = 0; j < ubdDetails_length; j++){
...
describe('Staff checkout Redemption Order', function() {
  it("Checkout Redemption order", () => {
    ...
    .should(response => {
      ...
      //expect(items).to.have.length(1) //blm karena nda blm bisa apus ubd null
      //check order status PAID
      //check payment status PAID
      //check order by nik employee
      //check type nya order redemption
      //check isscanner true
      //check point.redeem 160
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if(items[i].sku === sku_redemption_1){
          ...
          for(let j = 0; j < ubdDetails_length; j++){
            ...
            if(yearExpiredResponse === yearExpiredTest && monthExpiredResponse === monthExpiredTest){
              expect(items[i].ubdDetail[j].total, `Total product ${sku_redemption_1} with UBD ${ubd_redemption_1} should be 1`).to.equal(1)
            }
          }
        } else {
          expect(items[i].sku, `SKU should be ${sku_redemption_2}`).to.equal(sku_redemption_2)
          ...
          const ubdDetails_length = items[i].ubdDetail.length
          for(let j = 0; j < ubdDetails_length; j++){
            expect(items[i].ubdDetail[j].total, `Total product ${sku_redemption_2} with UBD ${ubd_redemption_2} should be 1`).to.equal(1)
          }
        }
        ...
        const calculate_subtotal_pts = pts_redemption * items_qty
        console.log('Calculate Subtotal: ' + calculate_subtotal_pts);
        expect(items[i].subtotal_redemption_point, `Subtotal product ${items[i].sku} should be ${calculate_subtotal_pts}`).to.equal(calculate_subtotal_pts)
        pointsArray.push(parseInt(calculate_subtotal_pts, 10)); // Convert to integer
      }

      const calculate_pts = pointsArray.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
      console.log('calculate_pts: ' + calculate_pts);
      expect(calculate_pts, `Calculate point should be ${total_redemption_cart} the same as in cart`).to.equal(total_redemption_cart)
      expect(data.point.redeem, `Total Redemption in order should be ${calculate_pts}`).to.equal(calculate_pts)
    })
  })
  ...
  it("Check user point after redemption", () => {
    ...
    // const sku_redemption_1_pts = sku_redemption_1_data.sub_total
    ...
    .should(response => {
      const data = response.body.data
      expect(response.status, 'Response code should be 201').to.equal(201)
      expect(data.currentPoint, `User point should be ${pts_akhir} after transaction`).to.equal(pts_akhir)
    })
  })
  ...
  it(`Get product stock from Stock Summary ${sku_redemption_1} after redemption`, () => {
    const ubd = '2024-10-01'
    const url = URL_PRODUCT + '/admin/stock-summary'
    const urlFilter = url + `?sku=${sku_redemption_1}&page=1&limit=100&ubd=${ubd}&storeCode=${store_code}`
    //const sku_redemption_1_data = Cypress.env("sku_redemption_1_DATA")
    ...
    .should(response => {
      ...
      expect(docs.qty, `Qty summary SKU ${sku_redemption_1} should be ${qty_summary_calculate}`).to.equal(qty_summary_calculate)
  ...
  it(`Get product stock from Stock Summary ${sku_redemption_2} after redemption`, () => {
    const ubd = 'null'
    const url = URL_PRODUCT + '/admin/stock-summary'
    const urlFilter = url + `?sku=${sku_redemption_2}&page=1&limit=100&ubd=${ubd}&storeCode=${store_code}`
    //const sku_redemption_2_data = Cypress.env("sku_redemption_2_DATA")
    //const qty_sku_redemption_2 = sku_redemption_2_data.qty
    ...
    .should(response => {
      ...
      expect(data.totalDocs).to.equal(1)
      expect(docs.qty, `Qty summary SKU ${sku_redemption_2} should be ${qty_summary_calculate}`).to.equal(qty_summary_calculate)
      //dapetin qty di cart dikurangin sama summary
    })
  })
  ...
  it(`Get product stock from Stock Movement ${sku_redemption_1} after redemption`, () => {
    ...
    .should(response => {
      ...
      expect(response.status).to.equal(200)
      expect(docs.qty, `Qty in movement should be ${qty_sku_redemption_1}`).to.equal(qty_sku_redemption_1)
      expect(docs.totalStock, `Total Stock should be ${qty_movement_calculate}`).to.equal(qty_movement_calculate)
  ...
  it(`Get product stock from Stock Movement ${sku_redemption_2} after redemption`, () => {
    const url = URL_PRODUCT + '/admin/stock-movement'
    const urlFilter = url + `?sku=${sku_redemption_2}&page=1&limit=10&sort=${sort_desc}&ubd=${ubd_redemption_2}&from=${store_code}`
    //const urlFilter = url + `?sku=${sku_redemption_2}&page=1&limit=10&sort=${sort_desc}&from=${store_code}`
    //const sku_redemption_2_data = Cypress.env("sku_redemption_2_DATA")
    //const qty_sku_redemption_2 = sku_redemption_2_data.qty
    ...
    .should(response => {
      ...
      expect(response.status).to.equal(200)
      expect(docs.qty, `Qty in movement should be ${qty_sku_redemption_2}`).to.equal(qty_sku_redemption_2)
      expect(docs.totalStock, `Total Stock should be ${qty_movement_calculate}`).to.equal(qty_movement_calculate)
      //qty di cart redemption
      //ambil total stock ambil stock awal
    })
