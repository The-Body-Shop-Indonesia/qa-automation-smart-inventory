    ...
    it(`Add same product with different UBD ${different_UBD} for SKU ${sku_redemption_1}`, () => {
        ...
        .should(response => {
            ...
            if(items[i].sku === sku_redemption_1){
                ...
                const ubdDetails_length = items[i].ubdDetail.length
                expect(items[i].ubdDetail, `UBD Detail for ${sku_redemption_1} should be 2`).to.have.length(2)
                for(let j = 0; j < ubdDetails_length; j++){
        ...
        .should(response => {
            ...
            if(items[i].sku === sku_redemption_2){
                ...
                if(yearExpiredResponse === yearExpiredTest && monthExpiredResponse === monthExpiredTest){
                    ...
                    expect(items[i].ubdDetail[j].total, `Total product ${sku_redemption_1} with UBD ${different_UBD} should be 1`).to.equal(1)
                }
                }
            }
